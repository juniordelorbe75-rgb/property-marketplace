...
def login_user(
    ...
    # Issue a full session only after
    # any configured second factor succeeds.
    from backend.services.account_security_service import (
        finish_primary_auth,
    )

    return finish_primary_auth(
        db,
        user,
    )
...
def update_current_user(
    ...
    seller_phone_verification_token: str | None = None,
):
    user = (
        user_repository
        .get_user_by_id(
    ...
    if seller_updates:
        ...
        if phone_changed:
            if (
                not seller_phone_verification_token
            ):
                raise HTTPException(
                    status_code=422,
                    detail=(
                        "Debe verificar el nuevo "
